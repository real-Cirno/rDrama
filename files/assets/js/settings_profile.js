...
function updatebgselection(){
	...
	const backgrounds = [
		...
		for (i=0; i < bgsToDisplay.length; i++) {
			...
			str += `<button class="btn btn-secondary bg-button"><img class='bg-image' src="/i/backgrounds/${bgsDir}/${bgsToDisplay[i]}?v=3" alt="${bgsToDisplay[i]}-background" onclick="post('/settings/profile?background=${onclickPost}')"></button>`;
		}
